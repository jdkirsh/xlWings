...
def json2csv_file(json_in_file):
    try:
        with open(json_in_file, 'r') as f:
           raw_data = json.load(f)
    except EnvironmentError:
        print(json_in_file, "file not found")

    csv_outfile = (os.path.splitext(json_in_file)[0]) + '.csv'
    try:
        csv_data = open(csv_outfile, 'w')
    except EnvironmentError:
        print(csv_outfile, "I/O Error")

    headers, processed_data = reduce_json(raw_data, minimize_columns=False)
    ...
    # output = convert_to_csv(raw_data, output=csv_outfile, minimize_columns=False)
    # output.close()
